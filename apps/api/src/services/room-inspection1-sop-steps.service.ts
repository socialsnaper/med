...
import type {
  CreateRoomInspection1SopStepInput,
  UpdateRoomInspection1SopStepInput,
  Insp1ImportRow,
} from '../validation/room-inspection1-sop-steps.schemas';

// ── Custom error ───────────────────────────────────────────────────────────────

export class RoomInspection1SopStepError extends Error {
  constructor(
    message: string,
    public readonly statusCode: number = 400,
    public readonly code: string = 'ROOM_INSPECTION1_SOP_STEP_ERROR',
  ) {
    super(message);
    this.name = 'RoomInspection1SopStepError';
  }
}

// ── Types ──────────────────────────────────────────────────────────────────────

export interface RoomInspection1SopStepMediaItem {
  id:           string;
  sopStepId:    string;
  displayOrder: number;
  fileUrl:      string;
...
export interface RoomInspection1SopStepItem {
  ...
  status:           string;
  media:            RoomInspection1SopStepMediaItem[];
  createdAt:        Date;
  updatedAt:        Date;
}
...
const SELECT = {
  ...
  media: {
    select: {
      ...
      fileType: true, caption: true, displayOrder: true, createdAt: true,
    },
...
function mapRow(row: any): RoomInspection1SopStepItem {
  return {
    id:               row.id,
    slid:             row.slid,
...
async function nextSlid(cleaningTypeId: string, schemaName: string): Promise<number> {
  ...
  const agg = await db.roomInspection1SopStep.aggregate({
...
async function renumberSlids(cleaningTypeId: string, schemaName: string): Promise<void> {
  const db   = getPrismaClient(schemaName);
  const rows = await db.roomInspection1SopStep.findMany({
    where:   { cleaningTypeId },
    select:  { id: true },
    orderBy: { stepNumber: 'asc' },
  });
  for (let i = 0; i < rows.length; i++) {
    await db.roomInspection1SopStep.update({
      where: { id: rows[i].id },
      data:  { slid: i + 1 },
    });
  }
...
export async function listRoomInspection1SopSteps(
  schemaName: string,
  cleaningTypeId?: string,
  status?: string,
...
export async function getRoomInspection1SopStep(
  id: string, schemaName: string,
): Promise<RoomInspection1SopStepItem> {
  const db  = getPrismaClient(schemaName);
  const row = await db.roomInspection1SopStep.findUnique({ select: SELECT, where: { id } });
  if (!row) throw new RoomInspection1SopStepError('Inspection step not found', 404, 'NOT_FOUND');
  return mapRow(row);
}

export async function createRoomInspection1SopStep(
  dto: CreateRoomInspection1SopStepInput,
  schemaName: string,
  userId: string,
): Promise<RoomInspection1SopStepItem> {
  const db = getPrismaClient(schemaName);
  ...
  if (!ct) throw new RoomInspection1SopStepError('Cleaning type not found', 404, 'CT_NOT_FOUND');

  const clash = await db.roomInspection1SopStep.findFirst({
    where: { cleaningTypeId: dto.cleaningTypeId, stepNumber: dto.stepNumber },
    select: { id: true },
  });
  if (clash) throw new RoomInspection1SopStepError(
  ...
  const row = await db.roomInspection1SopStep.create({
    select: SELECT,
    data: {
      slid,
...
export async function updateRoomInspection1SopStep(
  id: string,
  dto: UpdateRoomInspection1SopStepInput,
  schemaName: string,
  userId: string,
): Promise<RoomInspection1SopStepItem> {
  const db       = getPrismaClient(schemaName);
  const existing = await db.roomInspection1SopStep.findUnique({
    where: { id }, select: { id: true, cleaningTypeId: true },
  });
  if (!existing) throw new RoomInspection1SopStepError('Inspection step not found', 404, 'NOT_FOUND');
  ...
  const row = await db.roomInspection1SopStep.update({
    select: SELECT,
    where:  { id },
    data: {
...
export async function deleteRoomInspection1SopStep(id: string, schemaName: string): Promise<void> {
  const db = getPrismaClient(schemaName);
  const existing = await db.roomInspection1SopStep.findUnique({
    where: { id }, select: { id: true, cleaningTypeId: true },
  });
  if (!existing) throw new RoomInspection1SopStepError('Inspection step not found', 404, 'NOT_FOUND');
  await db.roomInspection1SopStep.delete({ where: { id } });
...
export async function streamRoomInspection1SopStepsCsv(
  schemaName: string,
  res: Response,
  cleaningTypeId?: string,
): Promise<void> {
  const rows = await listRoomInspection1SopSteps(schemaName, cleaningTypeId);

  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'attachment; filename="room_inspection1_sop_steps.csv"');
  res.write(CSV_HEADER + '\n');
  for (const r of rows) {
    res.write(csvRow([r.cleaningTypeCode, r.stepNumber, r.procedureText, r.status]) + '\n');
  }
  res.end();
}
...
export async function importRoomInspection1SopSteps(
  rows: Insp1ImportRow[],
  schemaName: string,
  userId: string,
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      await createRoomInspection1SopStep(
        { cleaningTypeId: ct.id, stepNumber: row.stepNumber, procedureText: row.procedureText, status: 'approved' },
        schemaName,
        userId,
      );
      created++;
