...
export const AddEquSopMediaSchema = z.object({
  fileUrl:      z.string()
    .max(500)
    .refine(
      (url) =>
  ...
  displayOrder: z.number().int().min(1).max(3).optional(),
});

export type AddEquSopMediaInput = z.infer<typeof AddEquSopMediaSchema>;
