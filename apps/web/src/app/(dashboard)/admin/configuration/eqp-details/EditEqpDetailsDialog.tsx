...
interface Props {
  item:      EquipmentDetailItem | null
  onClose:   () => void
  onUpdated: (item: EquipmentDetailItem) => void
}
...
export function EditEqpDetailsDialog({ item, onClose, onUpdated }: Props) {
  ...
  useEffect(() => {
    if (item) {
      form.reset({
        ...
        equipmentType:      item.equipmentType      as typeof EQUIPMENT_TYPES[number],
        manufacturer:       item.manufacturer       ?? "",
        purchaseDate:       fmtDate(item.purchaseDate),
        commissionDate:     fmtDate(item.commissionDate),
        decommissionDate:   fmtDate(item.decommissionDate),
  ...
  async function onSubmit(values: FormValues) {
    ...
    setSubmitError(null)
    try {
      const updated = await apiUpdateEquipmentDetail(token, item.id, {
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Pencil className="size-5 text-primary" /> Edit Equipment
          </DialogTitle>
          <DialogDescription>
            ID: <span className="font-mono font-medium">{item?.equipmentId}</span>
          </DialogDescription>
        </DialogHeader>
