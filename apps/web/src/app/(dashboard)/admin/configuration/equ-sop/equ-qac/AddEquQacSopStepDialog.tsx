...
import { apiCreateEquQacSopStep, apiUploadEquQacMediaFile, apiAddEquQacMedia, type EquQacSopStepItem, type RoomCleaningTypeItem, ApiError } from "@/lib/auth/api"
...
interface Props { open: boolean; onClose: () => void; onCreated: (i: EquQacSopStepItem) => void; cleaningTypes: RoomCleaningTypeItem[]; defaultTypeId?: string }
export function AddEquQacSopStepDialog({ open, onClose, onCreated, cleaningTypes, defaultTypeId }: Props) {
  ...
  async function onSubmit(v: FV) {
    ...
    try {
      const created = await apiCreateEquQacSopStep(t, { cleaningTypeId: v.cleaningTypeId, stepNumber: v.stepNumber, procedureText: v.procedureText })
      const media = [...(created.media ?? [])]
      for (const pic of pending) { try { const u = await apiUploadEquQacMediaFile(t, pic.file); media.push(await apiAddEquQacMedia(t, created.id, { fileUrl: u.url, fileName: u.fileName, fileType: u.fileType })) } catch { /* non-fatal */ } }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><FlaskConical className="size-5 text-primary" /> Add QAC Step</DialogTitle><DialogDescription>Add an equipment QAC SOP step.</DialogDescription></DialogHeader>
