...
import { apiAddEquInsp1Media, apiDeleteEquInsp1Media, apiUploadEquInsp1MediaFile, type EquInsp1SopStepItem, type EquInsp1MediaItem, ApiError } from "@/lib/auth/api"
...
interface Props { step: EquInsp1SopStepItem | null; onClose: () => void; onMediaChange: (id: string, m: EquInsp1MediaItem[]) => void }
export function ManagePicturesDialog({ step, onClose, onMediaChange }: Props) {
  const { getAccessToken } = useAuth()
  const [media, setMedia] = useState<EquInsp1MediaItem[]>(step?.media ?? [])
  ...
  async function handleAdd() {
    ...
    try {
      ...
      if (addMode === "file") { if (!selFile) { setAddError("Choose a file"); setIsAdding(false); return }; const r = await apiUploadEquInsp1MediaFile(t, selFile); fileUrl = r.url; fileName = r.fileName; fileType = r.fileType }
      ...
      const created = await apiAddEquInsp1Media(t, step.id, { fileUrl, fileName, fileType, caption: caption.trim() || undefined })
  ...
  async function handleDelete(mediaId: string) {
    ...
    try { await apiDeleteEquInsp1Media(t, step.id, mediaId); const upd = media.filter((m) => m.id !== mediaId); setMedia(upd); onMediaChange(step.id, upd) } catch (err) { setDeleteError(err instanceof ApiError ? err.message : "Failed to delete") } finally { setDeletingId(null) }
  }
  ...
  return (
    <Dialog open={!!step} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><ImageIcon className="size-5 text-emerald-600" /> Pictures</DialogTitle><DialogDescription>{step.cleaningTypeName} — Step {step.stepNumber} <span className="ml-2 text-xs text-amber-600">(min. 1 required for approved steps)</span></DialogDescription></DialogHeader>
