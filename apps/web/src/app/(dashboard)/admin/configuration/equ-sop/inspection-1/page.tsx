...
import {
  apiListEquInsp1SopSteps, apiDeleteEquInsp1SopStep,
  apiExportEquInsp1SopSteps, apiImportEquInsp1SopSteps,
  apiListRoomCleaningTypes,
  type EquInsp1SopStepItem, type RoomCleaningTypeItem, type EquInsp1MediaItem, ApiError,
} from "@/lib/auth/api"
import { Button } from "@/components/ui/button"
import { AddEquInsp1SopStepDialog }   from "./AddEquInsp1SopStepDialog"
import { EditEquInsp1SopStepDialog }  from "./EditEquInsp1SopStepDialog"
...
function downloadTemplate() { const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([TEMPLATE], { type: "text/csv" })); a.download = "equ_inspection1_sop_steps_template.csv"; a.click() }
...
export default function EquInspection1SopPage() {
  ...
  const [editStep,     setEditStep]     = useState<EquInsp1SopStepItem | null>(null)
  const [pictureStep,  setPictureStep]  = useState<EquInsp1SopStepItem | null>(null)
  ...
  const load = useCallback(async (typeId?: string) => { const t = getAccessToken(); if (!t) return; setIsLoading(true); setError(null); try { setSteps(await apiListEquInsp1SopSteps(t, typeId || undefined)) } catch (err) { setError(err instanceof ApiError ? err.message : "Failed to load") } finally { setIsLoading(false) } }, [getAccessToken])
  useEffect(() => { loadTypes(); load() }, [loadTypes, load])

  function handleTypeFilter(id: string) { setActiveTypeId(id); load(id || undefined) }
  async function handleDelete(id: string) { const t = getAccessToken(); if (!t) return; setIsDeleting(true); setActionError(null); try { await apiDeleteEquInsp1SopStep(t, id); await load(activeTypeId || undefined); setDeleteId(null) } catch (err) { setActionError(err instanceof ApiError ? err.message : "Delete failed") } finally { setIsDeleting(false) } }
  async function handleExport() { const t = getAccessToken(); if (!t) return; setIsExporting(true); try { const blob = await apiExportEquInsp1SopSteps(t, activeTypeId || undefined); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "equ_inspection1_sop_steps.csv"; a.click() } catch { /* */ } finally { setIsExporting(false) } }
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) { const file = e.target.files?.[0]; if (!file) return; e.target.value = ""; const t = getAccessToken(); if (!t) return; const rows = parseCsv(await file.text()); if (!rows.length) { setActionError("No valid rows found."); return } setIsImporting(true); try { const result = await apiImportEquInsp1SopSteps(t, rows); setImportResult(result); if (result.created > 0) load(activeTypeId || undefined) } catch (err) { setActionError(err instanceof ApiError ? err.message : "Import failed") } finally { setIsImporting(false) } }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-emerald-500/10"><ClipboardCheck className="size-5 text-emerald-600" /></div>
        <div><h1 className="text-2xl font-semibold tracking-tight">Equ Inspection 1 SOP</h1><p className="text-sm text-muted-foreground">Manage 1st inspection steps after equipment cleaning</p></div>
      </div>
      ...
      <ManagePicturesDialog step={pictureStep} onClose={() => setPictureStep(null)} onMediaChange={(id, media: EquInsp1MediaItem[]) => { setSteps((p) => p.map((s) => s.id === id ? { ...s, media } : s)); if (pictureStep?.id === id) setPictureStep((p) => p ? { ...p, media } : null) }} />
      <AddEquInsp1SopStepDialog open={addOpen} onClose={() => setAddOpen(false)} onCreated={(item) => setSteps((p) => [...p, item].sort((a, b) => a.cleaningTypeId.localeCompare(b.cleaningTypeId) || a.stepNumber - b.stepNumber))} cleaningTypes={cleaningTypes} defaultTypeId={activeTypeId || undefined} />
      <EditEquInsp1SopStepDialog item={editStep} onClose={() => setEditStep(null)} onUpdated={(u) => { setSteps((p) => p.map((s) => s.id === u.id ? u : s)); setEditStep(null) }} onMediaChange={(id, media) => setSteps((p) => p.map((s) => s.id === id ? { ...s, media } : s))} />
    </div>
  )
}
