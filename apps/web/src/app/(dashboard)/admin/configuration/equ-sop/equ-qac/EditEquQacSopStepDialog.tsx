...
import { apiUpdateEquQacSopStep, apiUploadEquQacMediaFile, apiAddEquQacMedia, apiDeleteEquQacMedia, EQU_QAC_STATUSES, type EquQacSopStepItem, type EquQacMediaItem, ApiError } from "@/lib/auth/api"
const SL: Record<string, string> = { pending: "Pending", approved: "Approved", rejected: "Rejected", archived: "Archived" }
const schema = z.object({ stepNumber: z.coerce.number().int().min(1), procedureText: z.string().min(1).max(5000), status: z.enum(EQU_QAC_STATUSES) })
type FV = z.infer<typeof schema>
interface Props { item: EquQacSopStepItem | null; onClose: () => void; onUpdated: (i: EquQacSopStepItem) => void; onMediaChange?: (id: string, m: EquQacMediaItem[]) => void }
export function EditEquQacSopStepDialog({ item, onClose, onUpdated, onMediaChange }: Props) {
  const { getAccessToken } = useAuth()
  const [submitError, setSubmitError] = useState<string | null>(null)
  const [localMedia, setLocalMedia]   = useState<EquQacMediaItem[]>([])
  ...
  useEffect(() => { if (item) { setLocalMedia(item.media ?? []); setPicError(null); form.reset({ stepNumber: item.stepNumber, procedureText: item.procedureText, status: item.status as typeof EQU_QAC_STATUSES[number] }); setSubmitError(null) } }, [item]) // eslint-disable-line react-hooks/exhaustive-deps
  async function handleFileAdd(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try { const u = await apiUploadEquQacMediaFile(t, file); const m = await apiAddEquQacMedia(t, item.id, { fileUrl: u.url, fileName: u.fileName, fileType: u.fileType }); const upd = [...localMedia, m]; setLocalMedia(upd); onMediaChange?.(item.id, upd) } catch (err) { setPicError(err instanceof ApiError ? err.message : "Upload failed") } finally { setIsUploading(false) }
  }
  async function handleDeleteMedia(mediaId: string) {
    if (!item) return; const t = getAccessToken(); if (!t) return; setDeletingId(mediaId); setPicError(null)
    try { await apiDeleteEquQacMedia(t, item.id, mediaId); const upd = localMedia.filter((m) => m.id !== mediaId); setLocalMedia(upd); onMediaChange?.(item.id, upd) } catch (err) { setPicError(err instanceof ApiError ? err.message : "Delete failed") } finally { setDeletingId(null) }
  }
  async function onSubmit(v: FV) {
    if (!item) return; const t = getAccessToken(); if (!t) return; setSubmitError(null)
    try { const u = await apiUpdateEquQacSopStep(t, item.id, { stepNumber: v.stepNumber, procedureText: v.procedureText, status: v.status }); onUpdated({ ...u, media: localMedia }); onClose() } catch (err) { setSubmitError(err instanceof ApiError ? err.message : "Unexpected error") }
  }
  function handleClose() { setSubmitError(null); onClose() }
  return (
    <Dialog open={!!item} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><Pencil className="size-5 text-primary" /> Edit QAC Step</DialogTitle><DialogDescription>{item?.cleaningTypeName} â€” Step {item?.stepNumber}</DialogDescription></DialogHeader>
