...
import { apiCreateEquInsp1SopStep, apiUploadEquInsp1MediaFile, apiAddEquInsp1Media, type EquInsp1SopStepItem, type RoomCleaningTypeItem, ApiError } from "@/lib/auth/api"
...
interface Props { open: boolean; onClose: () => void; onCreated: (i: EquInsp1SopStepItem) => void; cleaningTypes: RoomCleaningTypeItem[]; defaultTypeId?: string }
export function AddEquInsp1SopStepDialog({ open, onClose, onCreated, cleaningTypes, defaultTypeId }: Props) {
  ...
  async function onSubmit(v: FV) {
    ...
    try {
      const created = await apiCreateEquInsp1SopStep(t, { cleaningTypeId: v.cleaningTypeId, stepNumber: v.stepNumber, procedureText: v.procedureText })
      const media = [...(created.media ?? [])]
      for (const pic of pending) { try { const u = await apiUploadEquInsp1MediaFile(t, pic.file); media.push(await apiAddEquInsp1Media(t, created.id, { fileUrl: u.url, fileName: u.fileName, fileType: u.fileType })) } catch { /* non-fatal */ } }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><ClipboardCheck className="size-5 text-primary" /> Add Inspection 1 Step</DialogTitle><DialogDescription>Add an equipment inspection 1 SOP step.</DialogDescription></DialogHeader>
