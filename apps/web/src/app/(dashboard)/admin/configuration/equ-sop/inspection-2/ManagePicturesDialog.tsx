...
import { apiAddEquInsp2Media, apiDeleteEquInsp2Media, apiUploadEquInsp2MediaFile, type EquInsp2SopStepItem, type EquInsp2MediaItem, ApiError } from "@/lib/auth/api"
...
interface Props { step: EquInsp2SopStepItem | null; onClose: () => void; onMediaChange: (id: string, m: EquInsp2MediaItem[]) => void }
export function ManagePicturesDialog({ step, onClose, onMediaChange }: Props) {
  const { getAccessToken } = useAuth()
  const [media, setMedia] = useState<EquInsp2MediaItem[]>(step?.media ?? [])
  ...
  async function handleAdd() {
    ...
    try {
      ...
      if (addMode === "file") { if (!selFile) { setAddError("Choose a file"); setIsAdding(false); return }; const r = await apiUploadEquInsp2MediaFile(t, selFile); fileUrl = r.url; fileName = r.fileName; fileType = r.fileType }
      ...
      const created = await apiAddEquInsp2Media(t, step.id, { fileUrl, fileName, fileType, caption: caption.trim() || undefined })
  ...
  async function handleDelete(mediaId: string) {
    ...
    try { await apiDeleteEquInsp2Media(t, step.id, mediaId); const upd = media.filter((m) => m.id !== mediaId); setMedia(upd); onMediaChange(step.id, upd) } catch (err) { setDeleteError(err instanceof ApiError ? err.message : "Failed to delete") } finally { setDeletingId(null) }
  }
  ...
  return (
    <Dialog open={!!step} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><ImageIcon className="size-5 text-orange-600" /> Pictures</DialogTitle><DialogDescription>{step.cleaningTypeName} â€” Step {step.stepNumber} <span className="ml-2 text-xs text-amber-600">(min. 1 required for approved steps)</span></DialogDescription></DialogHeader>
