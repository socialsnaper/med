...
import {
  apiListEquQacSopSteps, apiDeleteEquQacSopStep,
  apiExportEquQacSopSteps, apiImportEquQacSopSteps,
  apiListRoomCleaningTypes,
  type EquQacSopStepItem, type RoomCleaningTypeItem, type EquQacMediaItem, ApiError,
} from "@/lib/auth/api"
import { Button } from "@/components/ui/button"
import { AddEquQacSopStepDialog }   from "./AddEquQacSopStepDialog"
import { EditEquQacSopStepDialog }  from "./EditEquQacSopStepDialog"
...
function downloadTemplate() { const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([TEMPLATE], { type: "text/csv" })); a.download = "equ_qac_sop_steps_template.csv"; a.click() }
...
export default function EquQacSopPage() {
  ...
  const [editStep,     setEditStep]     = useState<EquQacSopStepItem | null>(null)
  const [pictureStep,  setPictureStep]  = useState<EquQacSopStepItem | null>(null)
  ...
  const load = useCallback(async (typeId?: string) => { const t = getAccessToken(); if (!t) return; setIsLoading(true); setError(null); try { setSteps(await apiListEquQacSopSteps(t, typeId || undefined)) } catch (err) { setError(err instanceof ApiError ? err.message : "Failed to load") } finally { setIsLoading(false) } }, [getAccessToken])
  useEffect(() => { loadTypes(); load() }, [loadTypes, load])

  function handleTypeFilter(id: string) { setActiveTypeId(id); load(id || undefined) }
  async function handleDelete(id: string) { const t = getAccessToken(); if (!t) return; setIsDeleting(true); setActionError(null); try { await apiDeleteEquQacSopStep(t, id); await load(activeTypeId || undefined); setDeleteId(null) } catch (err) { setActionError(err instanceof ApiError ? err.message : "Delete failed") } finally { setIsDeleting(false) } }
  async function handleExport() { const t = getAccessToken(); if (!t) return; setIsExporting(true); try { const blob = await apiExportEquQacSopSteps(t, activeTypeId || undefined); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "equ_qac_sop_steps.csv"; a.click() } catch { /* */ } finally { setIsExporting(false) } }
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) { const file = e.target.files?.[0]; if (!file) return; e.target.value = ""; const t = getAccessToken(); if (!t) return; const rows = parseCsv(await file.text()); if (!rows.length) { setActionError("No valid rows found."); return } setIsImporting(true); try { const result = await apiImportEquQacSopSteps(t, rows); setImportResult(result); if (result.created > 0) load(activeTypeId || undefined) } catch (err) { setActionError(err instanceof ApiError ? err.message : "Import failed") } finally { setIsImporting(false) } }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-semibold tracking-tight">Equ QAC SOP</h1><p className="text-sm text-muted-foreground">Manage QAC steps after equipment cleaning</p></div>
      </div>
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        ...
        : steps.length === 0 ? <div className="flex flex-col items-center justify-center py-16 gap-2"><FlaskConical className="size-8 text-muted-foreground/40" /><p className="text-sm text-muted-foreground">{activeTypeId ? "No steps for this type." : "No steps added yet."}</p></div>
      ...
      <ManagePicturesDialog step={pictureStep} onClose={() => setPictureStep(null)} onMediaChange={(id, media: EquQacMediaItem[]) => { setSteps((p) => p.map((s) => s.id === id ? { ...s, media } : s)); if (pictureStep?.id === id) setPictureStep((p) => p ? { ...p, media } : null) }} />
      <AddEquQacSopStepDialog open={addOpen} onClose={() => setAddOpen(false)} onCreated={(item) => setSteps((p) => [...p, item].sort((a, b) => a.cleaningTypeId.localeCompare(b.cleaningTypeId) || a.stepNumber - b.stepNumber))} cleaningTypes={cleaningTypes} defaultTypeId={activeTypeId || undefined} />
      <EditEquQacSopStepDialog item={editStep} onClose={() => setEditStep(null)} onUpdated={(u) => { setSteps((p) => p.map((s) => s.id === u.id ? u : s)); setEditStep(null) }} onMediaChange={(id, media) => setSteps((p) => p.map((s) => s.id === id ? { ...s, media } : s))} />
    </div>
  )
}
