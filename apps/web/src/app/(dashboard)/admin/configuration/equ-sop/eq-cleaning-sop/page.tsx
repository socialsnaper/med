...
import {
  apiListEquCleaningSopSteps, apiDeleteEquCleaningSopStep,
  apiExportEquCleaningSopSteps, apiImportEquCleaningSopSteps,
  apiListRoomCleaningTypes,
  type EquCleaningSopStepItem, type RoomCleaningTypeItem, type EquSopMediaItem,
  type EquSopImportRow, EQU_CLEANING_METHODS, ApiError,
} from "@/lib/auth/api"
import { Button } from "@/components/ui/button"
import { AddEquCleaningSopStepDialog } from "./AddEquCleaningSopStepDialog"
import { EditEquCleaningSopStepDialog } from "./EditEquCleaningSopStepDialog"
import { ManagePicturesDialog } from "./ManagePicturesDialog"
import {
  Wrench, ChevronLeft, Plus, Pencil, Trash2,
...
function MethodBadge({ method }: { method: string }) {
  return <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1", METHOD_COLORS[method] ?? "bg-slate-50 text-slate-600 ring-slate-200")}>{METHOD_LABELS[method] ?? method}</span>
}
function StatusBadge({ status }: { status: string }) {
  const Icon = status === "approved" ? CheckCircle2 : status === "rejected" ? XCircle : null
  return <span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ring-1 capitalize", STATUS_COLORS[status] ?? "bg-slate-50 text-slate-600 ring-slate-200")}>{Icon && <Icon className="size-3" />} {status}</span>
}
function PictureCellThumbnail({ url }: { url: string }) {
  const [broken, setBroken] = useState(false)
  if (broken) return <div className="w-12 h-10 flex items-center justify-center bg-muted rounded border"><ImageIcon className="size-4 text-muted-foreground/40" /></div>
  // eslint-disable-next-line @next/next/no-img-element
  return <img src={url} alt="reference" className="w-12 h-10 object-cover rounded border" onError={() => setBroken(true)} />
}

// ── CSV template ──────────────────────────────────────────────────────────────

const CSV_TEMPLATE = [
  "cleaning_type_code,step_number,time_allotted,cleaning_method,procedure_text,chemical_used,equipment_used",
  "RCT-001,1,00:05,TypeB,Wipe equipment surfaces with damp cloth.,Detergent Solution,Lint-free wipe",
  "RCT-001,2,00:10,TypeB,Rinse with purified water and dry.,NA,Absorbent cloth",
].join("\n")

function downloadTemplate() {
  const blob = new Blob([CSV_TEMPLATE], { type: "text/csv" })
  const url  = URL.createObjectURL(blob); const a = document.createElement("a")
  a.href = url; a.download = "equ_cleaning_sop_steps_template.csv"; a.click(); URL.revokeObjectURL(url)
}

function parseCsv(text: string): EquSopImportRow[] {
  const lines  = text.trim().split(/\r?\n/)
  const header = lines[0].split(",").map((h) => h.trim().toLowerCase().replace(/\s+/g, "_"))
  return lines.slice(1).map((line) => {
    const cols: Record<string, string> = {}
    line.split(",").forEach((v, i) => { cols[header[i] ?? `col${i}`] = v.trim().replace(/^"|"$/g, "") })
    const method = cols["cleaning_method"] ?? "TypeB"
    return {
      ...
      cleaningMethod:      (EQU_CLEANING_METHODS.includes(method as typeof EQU_CLEANING_METHODS[number]) ? method : "TypeB") as typeof EQU_CLEANING_METHODS[number],
      procedureText:       cols["procedure_text"] ?? "",
      chemicalUsed:        cols["chemical_used"]  || undefined,
      equipmentUsed:       cols["equipment_used"] || undefined,
    }
  }).filter((r) => r.cleaningTypeCode && r.stepNumber > 0 && r.procedureText)
}

function DeleteConfirm({ item, onConfirm, onCancel, isDeleting }: { item: EquCleaningSopStepItem; onConfirm: () => void; onCancel: () => void; isDeleting: boolean }) {
  return (
    <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <Button size="sm" variant="destructive" onClick={onConfirm} disabled={isDeleting}>{isDeleting && <Loader2 className="mr-1.5 size-3 animate-spin" />} Delete</Button>
...
function ImportBanner({ result, onClose }: { result: { created: number; skipped: number; errors: { row: number; message: string }[] }; onClose: () => void }) {
  return (
    <div className="rounded-xl border bg-card p-4 space-y-2">
      <div className="flex items-center justify-between"><p className="text-sm font-medium">Import complete</p><button onClick={onClose}><X className="size-4 text-muted-foreground" /></button></div>
      ...
      {result.errors.length > 0 && <ul className="text-xs text-destructive space-y-0.5 max-h-24 overflow-auto">{result.errors.map((e, i) => <li key={i}>Row {e.row}: {e.message}</li>)}</ul>}
    </div>
  )
}
...
export default function EquCleaningSopPage() {
  const { user, getAccessToken } = useAuth()
  const router = useRouter()
  const [steps,         setSteps]         = useState<EquCleaningSopStepItem[]>([])
  ...
  const [editStep,      setEditStep]      = useState<EquCleaningSopStepItem | null>(null)
  const [pictureStep,   setPictureStep]   = useState<EquCleaningSopStepItem | null>(null)
  ...
  const load = useCallback(async (typeId?: string) => {
    const token = getAccessToken(); if (!token) return; setIsLoading(true); setError(null)
    try { setSteps(await apiListEquCleaningSopSteps(token, typeId || undefined)) }
    catch (err) { setError(err instanceof ApiError ? err.message : "Failed to load equipment cleaning steps") }
  ...
  async function handleDelete(id: string) {
    const token = getAccessToken(); if (!token) return; setIsDeleting(true); setActionError(null)
    try { await apiDeleteEquCleaningSopStep(token, id); await load(activeTypeId || undefined); setDeleteId(null) }
    catch (err) { setActionError(err instanceof ApiError ? err.message : "Delete failed") }
    finally { setIsDeleting(false) }
  }

  async function handleExport() {
    const token = getAccessToken(); if (!token) return; setIsExporting(true)
    try {
      const blob = await apiExportEquCleaningSopSteps(token, activeTypeId || undefined)
      const url  = URL.createObjectURL(blob); const a = document.createElement("a")
      a.href = url; a.download = "equ_cleaning_sop_steps.csv"; a.click(); URL.revokeObjectURL(url)
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsImporting(true)
    try { const result = await apiImportEquCleaningSopSteps(token, rows); setImportResult(result); if (result.created > 0) load(activeTypeId || undefined) }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-sky-500/10"><Wrench className="size-5 text-sky-600" /></div>
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Eq Cleaning SOP</h1>
          <p className="text-sm text-muted-foreground">Manage equipment cleaning standard operating procedure steps</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {cleaningTypes.map((ct) => (
            <button key={ct.id} onClick={() => handleTypeFilter(ct.id)} className={cn("rounded-full px-3 py-1 text-xs font-medium transition-colors", activeTypeId === ct.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80")}>{ct.cleaningTypeName}</button>
          ))}
        </div>
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-muted-foreground">{activeTypeId ? "No steps for this cleaning type." : "No equipment cleaning steps added yet."}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/30">
                  ...
                  <th className="text-right px-4 py-3 font-medium text-muted-foreground w-14">Step</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground w-16">Time</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground">Type</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground">Procedure</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">Chemical</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">Equipment</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">View Links</th>
              ...
              <tbody className="divide-y">
                {steps.map((step) => (
                  <tr key={step.id} className="hover:bg-muted/20 transition-colors align-top">
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground">{step.timeAllottedDisplay ?? "—"}</td>
                    <td className="px-4 py-3"><MethodBadge method={step.cleaningMethod} /></td>
                    <td className="px-4 py-3 max-w-xs"><p className="line-clamp-2 text-xs">{step.procedureText}</p></td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">{step.chemicalUsed ?? "—"}</td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">{step.equipmentUsed ?? "—"}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => setPictureStep(step)} className="flex items-center gap-1.5 group" title="Manage pictures">
                        {step.media.length > 0 ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-xs text-muted-foreground/60 group-hover:text-sky-600 transition-colors"><Camera className="size-4" /> Add</span>
                        )}
                      </button>
                    </td>
      ...
      <ManagePicturesDialog
        ...
        onMediaChange={(stepId, updatedMedia: EquSopMediaItem[]) => {
