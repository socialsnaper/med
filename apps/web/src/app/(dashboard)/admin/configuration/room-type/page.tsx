...
const CSV_TEMPLATE = [
  "room_type_name,room_type_details,display_order",
  "Granulation Room,Room used for granulation processes,1",
  "Quality Control Room,QC testing and sampling room,2",
].join("\n")
...
function downloadTemplate() {
  ...
  a.href = url; a.download = "room_types_template.csv"; a.click()
  URL.revokeObjectURL(url)
}

function parseCsv(text: string): RoomTypeImportRow[] {
  const lines  = text.trim().split(/\r?\n/)
  const header = lines[0].split(",").map((h) => h.trim().toLowerCase())
  return lines.slice(1).map((line) => {
    const cols: Record<string, string> = {}
    line.split(",").forEach((v, i) => { cols[header[i] ?? `col${i}`] = v.trim().replace(/^"|"$/g, "") })
    const row: RoomTypeImportRow = { roomTypeName: cols["room_type_name"] ?? "" }
    if (cols["room_type_details"]) row.roomTypeDetails = cols["room_type_details"]
    if (cols["display_order"])     row.displayOrder    = Number(cols["display_order"]) || 0
    return row
  }).filter((r) => r.roomTypeName)
}
...
export default function RoomTypePage() {
  ...
  async function handleExport() {
    ...
    setIsExporting(true)
    try {
      const blob = await apiExportRoomTypes(token)
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement("a")
      a.href = url; a.download = "room_types.csv"; a.click()
      URL.revokeObjectURL(url)
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsImporting(true)
    try {
      const result = await apiImportRoomTypes(token, rows)
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <Button onClick={() => setAddOpen(true)}>
            <Plus className="mr-1.5 size-4" /> Add Room Type
          </Button>
        </div>
      </div>
