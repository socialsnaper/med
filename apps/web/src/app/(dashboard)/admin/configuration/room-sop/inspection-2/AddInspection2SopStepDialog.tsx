...
import { z } from "zod"
import { Loader2, ClipboardList, ImageIcon, Plus, X } from "lucide-react"
...
import {
  apiCreateRoomInspection2SopStep,
...
interface Props {
  ...
  onCreated: (item: RoomInspection2SopStepItem) => void
  cleaningTypes: RoomCleaningTypeItem[]; defaultTypeId?: string
}
interface PendingPic { id: string; file: File; previewUrl: string }

export function AddInspection2SopStepDialog({ open, onClose, onCreated, cleaningTypes, defaultTypeId }: Props) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { cleaningTypeId: defaultTypeId ?? "", stepNumber: 1, procedureText: "" },
  })
  ...
  async function onSubmit(values: FormValues) {
    ...
    setSubmitError(null)
    try {
      const created = await apiCreateRoomInspection2SopStep(token, { cleaningTypeId: values.cleaningTypeId, stepNumber: values.stepNumber, procedureText: values.procedureText })
      const uploadedMedia = [...(created.media ?? [])]
      for (const pic of pending) {
        try {
          const uploaded = await apiUploadInspection2MediaFile(token, pic.file)
          const media    = await apiAddInspection2Media(token, created.id, { fileUrl: uploaded.url, fileName: uploaded.fileName, fileType: uploaded.fileType })
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><ClipboardList className="size-5 text-primary" /> Add Inspection 2 Step</DialogTitle>
          <DialogDescription>Add a new Inspection 2 SOP step.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
            ...
            <FormField control={form.control} name="procedureText" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea placeholder="Describe what the inspector must check…" className="min-h-28 resize-none" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
