...
import {
  apiListRoomQacSopSteps, apiDeleteRoomQacSopStep,
  apiExportRoomQacSopSteps, apiImportRoomQacSopSteps,
  apiListRoomCleaningTypes,
  type RoomQacSopStepItem, type RoomCleaningTypeItem, type QacMediaItem, ApiError,
} from "@/lib/auth/api"
import { Button } from "@/components/ui/button"
import { AddRoomQacSopStepDialog } from "./AddRoomQacSopStepDialog"
import { EditRoomQacSopStepDialog } from "./EditRoomQacSopStepDialog"
import { ManagePicturesDialog } from "./ManagePicturesDialog"
import {
  FlaskConical, ChevronLeft, Plus, Pencil, Trash2,
  Loader2, AlertCircle, CheckCircle2, XCircle, Download,
  Upload, RefreshCw, X, ImageIcon, Camera,
} from "lucide-react"
import { cn } from "@/lib/utils"

type QacImportRow = { cleaningTypeCode: string; stepNumber: number; procedureText: string }
...
const CSV_TEMPLATE = ["cleaning_type_code,step_number,procedure_text", "RCT-001,1,Verify room meets QAC standards for cleanliness.", "RCT-001,2,Confirm all surfaces pass quality assurance check."].join("\n")

function downloadTemplate() {
  const blob = new Blob([CSV_TEMPLATE], { type: "text/csv" })
  const url  = URL.createObjectURL(blob); const a = document.createElement("a")
  a.href = url; a.download = "room_qac_sop_steps_template.csv"; a.click(); URL.revokeObjectURL(url)
}

function parseCsv(text: string): QacImportRow[] {
  const lines  = text.trim().split(/\r?\n/)
  const header = lines[0].split(",").map((h) => h.trim().toLowerCase().replace(/\s+/g, "_"))
  return lines.slice(1).map((line) => {
    const cols: Record<string, string> = {}
    line.split(",").forEach((v, i) => { cols[header[i] ?? `col${i}`] = v.trim().replace(/^"|"$/g, "") })
    return { cleaningTypeCode: cols["cleaning_type_code"] ?? "", stepNumber: parseInt(cols["step_number"] ?? "0", 10) || 0, procedureText: cols["procedure_text"] ?? "" }
  }).filter((r) => r.cleaningTypeCode && r.stepNumber > 0 && r.procedureText)
}

function DeleteConfirm({ item, onConfirm, onCancel, isDeleting }: { item: RoomQacSopStepItem; onConfirm: () => void; onCancel: () => void; isDeleting: boolean }) {
  return (
...
export default function RoomQacSopPage() {
  const { user, getAccessToken } = useAuth()
  const router = useRouter()
  const [steps,         setSteps]         = useState<RoomQacSopStepItem[]>([])
  ...
  const [editStep,      setEditStep]      = useState<RoomQacSopStepItem | null>(null)
  const [pictureStep,   setPictureStep]   = useState<RoomQacSopStepItem | null>(null)
  ...
  const loadTypes = useCallback(async () => { const token = getAccessToken(); if (!token) return; try { setCleaningTypes(await apiListRoomCleaningTypes(token)) } catch { /* silent */ } }, [getAccessToken])
  const load = useCallback(async (typeId?: string) => {
    const token = getAccessToken(); if (!token) return; setIsLoading(true); setError(null)
    try { setSteps(await apiListRoomQacSopSteps(token, typeId || undefined)) }
    catch (err) { setError(err instanceof ApiError ? err.message : "Failed to load QAC steps") }
  ...
  async function handleDelete(id: string) {
    ...
    try { await apiDeleteRoomQacSopStep(token, id); await load(activeTypeId || undefined); setDeleteId(null) }
  ...
  async function handleExport() {
    ...
    try {
      const blob = await apiExportRoomQacSopSteps(token, activeTypeId || undefined)
      const url  = URL.createObjectURL(blob); const a = document.createElement("a")
      a.href = url; a.download = "room_qac_sop_steps.csv"; a.click(); URL.revokeObjectURL(url)
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsImporting(true)
    try { const result = await apiImportRoomQacSopSteps(token, rows); setImportResult(result); if (result.created > 0) load(activeTypeId || undefined) }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/configuration/room-sop" className="flex items-center justify-center size-8 rounded-lg border hover:bg-accent transition-colors"><ChevronLeft className="size-4" /></Link>
        <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-violet-500/10"><FlaskConical className="size-5 text-violet-600" /></div>
        <div><h1 className="text-2xl font-semibold tracking-tight">Room QAC SOP</h1><p className="text-sm text-muted-foreground">Manage Quality Assurance Check steps for room cleaning</p></div>
      </div>
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-muted-foreground">{activeTypeId ? "No QAC steps for this cleaning type." : "No QAC steps added yet."}</p>
          </div>
        ) : (
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setPictureStep(step)} className="flex items-center gap-1.5 group" title="Manage pictures">
                        {step.media.length > 0 ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-xs text-muted-foreground/60 group-hover:text-violet-600 transition-colors"><Camera className="size-4" /> Add</span>
                        )}
                      </button>
                    </td>
      ...
      <ManagePicturesDialog
        ...
        onMediaChange={(stepId, updatedMedia: QacMediaItem[]) => {
