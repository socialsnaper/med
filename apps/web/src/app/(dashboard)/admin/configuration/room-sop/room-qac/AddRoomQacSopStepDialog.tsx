...
import { z } from "zod"
import { Loader2, FlaskConical, ImageIcon, Plus, X } from "lucide-react"
...
import {
  apiCreateRoomQacSopStep, apiUploadQacMediaFile, apiAddQacMedia,
  type RoomQacSopStepItem, type RoomCleaningTypeItem, ApiError,
} from "@/lib/auth/api"
...
export function AddRoomQacSopStepDialog({ open, onClose, onCreated, cleaningTypes, defaultTypeId }: Props) {
  ...
  function handleFileAdd(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (valid.length < files.length) setPicError("Some files were skipped — only JPEG, PNG, WebP, GIF are allowed"); else setPicError(null)
  ...
  const form = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { cleaningTypeId: defaultTypeId ?? "", stepNumber: 1, procedureText: "" } })

  async function onSubmit(values: FormValues) {
    const token = getAccessToken(); if (!token) return; setSubmitError(null)
    try {
      const created = await apiCreateRoomQacSopStep(token, { cleaningTypeId: values.cleaningTypeId, stepNumber: values.stepNumber, procedureText: values.procedureText })
      const uploadedMedia = [...(created.media ?? [])]
      for (const pic of pending) {
        try {
          const uploaded = await apiUploadQacMediaFile(token, pic.file)
          const media    = await apiAddQacMedia(token, created.id, { fileUrl: uploaded.url, fileName: uploaded.fileName, fileType: uploaded.fileType })
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><FlaskConical className="size-5 text-primary" /> Add QAC Step</DialogTitle>
          <DialogDescription>Add a new Room QAC SOP step.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
            ...
            <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium flex items-center gap-1.5"><ImageIcon className="size-4 text-muted-foreground" /> Pictures <span className="text-xs font-normal text-muted-foreground">(optional)</span></span>
                <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1 rounded-md border bg-background px-2 py-1 text-xs hover:bg-accent transition-colors"><Plus className="size-3" /> Add image</button>
              </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting && <Loader2 className="mr-1.5 size-4 animate-spin" />} Add Step</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
