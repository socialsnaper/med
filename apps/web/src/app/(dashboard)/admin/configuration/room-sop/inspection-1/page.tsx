...
import {
  apiListRoomInspection1SopSteps,
  apiDeleteRoomInspection1SopStep,
  apiExportRoomInspection1SopSteps,
  apiImportRoomInspection1SopSteps,
...
import { AddInspection1SopStepDialog } from "./AddInspection1SopStepDialog"
import { EditInspection1SopStepDialog } from "./EditInspection1SopStepDialog"
...
function downloadTemplate() {
  ...
  a.href = url; a.download = "room_inspection1_sop_steps_template.csv"; a.click()
  URL.revokeObjectURL(url)
}

function parseCsv(text: string): Insp1ImportRow[] {
...
function DeleteConfirm({
  ...
  item: RoomInspection1SopStepItem; onConfirm: () => void; onCancel: () => void; isDeleting: boolean
}) {
  return (
...
export default function Inspection1SopPage() {
  const { user, getAccessToken } = useAuth()
  const router = useRouter()

  const [steps,        setSteps]        = useState<RoomInspection1SopStepItem[]>([])
  ...
  const [editStep,     setEditStep]     = useState<RoomInspection1SopStepItem | null>(null)
  const [pictureStep,  setPictureStep]  = useState<RoomInspection1SopStepItem | null>(null)
  ...
  const load = useCallback(async (typeId?: string) => {
    ...
    try {
      setSteps(await apiListRoomInspection1SopSteps(token, typeId || undefined))
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Failed to load inspection steps")
    } finally {
      setIsLoading(false)
    }
  }, [getAccessToken])
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await apiDeleteRoomInspection1SopStep(token, id)
  ...
  async function handleExport() {
    ...
    setIsExporting(true)
    try {
      const blob = await apiExportRoomInspection1SopSteps(token, activeTypeId || undefined)
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement("a")
      a.href = url; a.download = "room_inspection1_sop_steps.csv"; a.click()
      URL.revokeObjectURL(url)
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsImporting(true)
    try {
      const result = await apiImportRoomInspection1SopSteps(token, rows)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Inspection 1 SOP</h1>
          <p className="text-sm text-muted-foreground">Manage post-cleaning inspection steps for Inspector 1</p>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-muted-foreground">
              {activeTypeId ? "No inspection steps for this cleaning type." : "No inspection steps added yet."}
            </p>
          </div>
        ) : (
                ...
                {steps.map((step) => (
                  <tr key={step.id} className="hover:bg-muted/20 transition-colors align-top">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => setPictureStep(step)}
                        className="flex items-center gap-1.5 group"
                        title="Manage pictures"
                      >
                        {step.media.length > 0 ? (
                          <>
                            <PictureCellThumbnail url={step.media[0].fileUrl} />
                            {step.media.length > 1 && (
                          ...
                          <span className="inline-flex items-center gap-1 text-xs text-muted-foreground/60 group-hover:text-emerald-600 transition-colors">
                      ...
                      {deleteId === step.id ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button
                            ...
                            onClick={() => { setDeleteId(step.id); setActionError(null) }}
                          >
      ...
      <ManagePicturesDialog
        ...
        onMediaChange={(stepId, updatedMedia: Insp1MediaItem[]) => {
          setSteps((prev) => prev.map((s) =>
            s.id === stepId ? { ...s, media: updatedMedia } : s,
          ))
          if (pictureStep?.id === stepId) {
            setPictureStep((prev) => prev ? { ...prev, media: updatedMedia } : null)
          }
        }}
      />

      <AddInspection1SopStepDialog
        open={addOpen}
        onClose={() => setAddOpen(false)}
        onCreated={(item) => {
          setSteps((prev) => [...prev, item].sort((a, b) =>
            a.cleaningTypeId.localeCompare(b.cleaningTypeId) || a.stepNumber - b.stepNumber,
          ))
        }}
      ...
      <EditInspection1SopStepDialog
        ...
        onMediaChange={(stepId, updatedMedia) => {
          setSteps((prev) => prev.map((s) =>
            s.id === stepId ? { ...s, media: updatedMedia } : s,
          ))
        }}
