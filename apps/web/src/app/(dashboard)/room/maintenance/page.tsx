...
const STATUS_CONFIG: Record<MaintenanceStatus, { label: string; className: string; icon: React.ElementType }> = {
  ...
  scheduled: { label: "Pending / Approved", className: "bg-blue-50 text-blue-700 ring-blue-200",         icon: Clock },
...
interface AddDialogProps { rooms: RoomItem[]; types: MaintenanceTypeItem[]; onClose: () => void; onSaved: () => void; getAccessToken: () => string | null }

function AddMaintenanceDialog({ rooms, types, onClose, onSaved, getAccessToken }: AddDialogProps) {
  const now = new Date()
  const localIso = new Date(now.getTime() - now.getTimezoneOffset() * 60000).toISOString().slice(0, 16)
  const [roomId, setRoomId] = useState(""); const [maintenanceTypeId, setMaintenanceTypeId] = useState("")
  const [startDatetime, setStartDatetime] = useState(localIso); const [reason, setReason] = useState("")
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      await apiCreateMaintenance(token, { roomId, maintenanceTypeId, maintenanceStartDatetime: new Date(startDatetime).toISOString(), reasonForMaintenance: reason.trim() })
      onSaved()
    } catch (err) { setError(err instanceof ApiError ? err.message : "Failed to create maintenance record.") }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-background rounded-xl border shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div className="flex items-center gap-2"><Wrench className="size-4 text-sky-600" /><h2 className="font-semibold text-base">Add Room for Maintenance</h2></div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-700">
            The room will be blocked immediately. A System Administrator will be notified to approve or reject.
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">Room <span className="text-red-500">*</span></label>
            <select value={roomId} onChange={(e) => setRoomId(e.target.value)} className="h-9 w-full rounded-md border bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" required>
              <option value="">— Select Room —</option>
              {availableRooms.map((r) => <option key={r.id} value={r.id}>{r.roomId} — {r.roomName}{r.floor ? ` (Floor ${r.floor})` : ""}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {activeTypes.map((t) => (
                <button key={t.id} type="button" onClick={() => setMaintenanceTypeId(t.id)}
                  className={cn("rounded-full border px-3 py-1 text-sm font-medium transition-colors", maintenanceTypeId === t.id ? "border-sky-600 bg-sky-50 text-sky-700" : "border-border text-muted-foreground hover:border-sky-400")}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="datetime-local" value={startDatetime} onChange={(e) => setStartDatetime(e.target.value)} className="h-9 w-full rounded-md border bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" required />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">Reason / Remarks <span className="text-red-500">*</span></label>
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder="Describe the reason for maintenance..." className="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none" required />
          </div>
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="submit" size="sm" disabled={submitting} className="bg-sky-600 hover:bg-sky-700 text-white">
              {submitting ? <><Loader2 className="size-3.5 mr-1.5 animate-spin" /> Saving…</> : <><Plus className="size-3.5 mr-1.5" /> Submit Request</>}
            </Button>
          </div>
        </form>
      </div>
...
interface StartDialogProps { log: MaintenanceLogItem; onClose: () => void; onSaved: () => void; getAccessToken: () => string | null }
...
function StartMaintenanceDialog({ log, onClose, onSaved, getAccessToken }: StartDialogProps) {
  ...
  async function handleStart() {
    ...
    setSubmitting(true)
    try { await apiStartMaintenance(token, log.id); onSaved() }
...
interface StopDialogProps { log: MaintenanceLogItem; onClose: () => void; onSaved: () => void; getAccessToken: () => string | null }

function StopMaintenanceDialog({ log, onClose, onSaved, getAccessToken }: StopDialogProps) {
  const [remarks, setRemarks] = useState(""); const [submitting, setSubmitting] = useState(false); const [error, setError] = useState<string | null>(null)
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); setError(null); const token = getAccessToken(); if (!token) { setError("Session expired."); return }
    setSubmitting(true)
    try { await apiStopMaintenance(token, log.id, { completionRemarks: remarks.trim() || undefined }); onSaved() }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-background rounded-xl border shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          ...
          <p className="text-sm text-muted-foreground">The room will be restored to active status after stopping.</p>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">Completion Remarks</label>
            <textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} rows={3} placeholder="Optional — describe what was done..." className="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none" />
          </div>
          <div className="flex justify-end gap-2 pt-1">
            <Button type="button" variant="outline" size="sm" onClick={onClose} disabled={submitting}>Cancel</Button>
...
interface AuthDialogProps { log: MaintenanceLogItem; mode: "approve" | "reject"; onClose: () => void; onSaved: () => void; getAccessToken: () => string | null }
...
function AuthDialog({ log, mode, onClose, onSaved, getAccessToken }: AuthDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      if (mode === "approve") await apiApproveMaintenance(token, log.id, { authorizationRemarks: remarks.trim() || undefined })
      else await apiRejectMaintenance(token, log.id, { authorizationRemarks: remarks.trim() })
      onSaved()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-background rounded-xl border shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          ...
          {isApprove && <p className="text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">Approving will allow the Cleaning Operator to start this maintenance.</p>}
          {!isApprove && <p className="text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2">Rejecting will cancel this maintenance and restore the room to active.</p>}
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">Remarks {!isApprove && <span className="text-red-500">*</span>}</label>
            <textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} rows={3} placeholder={isApprove ? "Optional remarks..." : "Required — reason for rejection..."} className="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none" required={!isApprove} />
          </div>
          <div className="flex justify-end gap-2 pt-1">
            <Button type="button" variant="outline" size="sm" onClick={onClose} disabled={submitting}>Cancel</Button>
...
export default function RoomMaintenancePage() {
  ...
  const [startTarget, setStartTarget] = useState<MaintenanceLogItem | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [logsData, roomsData, typesData] = await Promise.all([apiListMaintenanceLogs(token), apiListRooms(token), apiListMaintenanceTypes(token)])
      setLogs(logsData); setRooms(roomsData); setTypes(typesData)
  ...
  const isCleaningOperator = user?.role === "Cleaning Operator"

  // Cleaning Operator only sees approved records (server already filters, this is defense-in-depth)
  const visibleLogs = isCleaningOperator
    ? logs.filter((l) =>
  ...
  const filtered = visibleLogs.filter((l) => {
    ...
    const matchSearch = !q || l.roomName.toLowerCase().includes(q) || l.roomCode.toLowerCase().includes(q) || l.maintenanceTypeName.toLowerCase().includes(q) || l.reasonForMaintenance.toLowerCase().includes(q)
  ...
  function onDialogSaved() { setAddOpen(false); setStartTarget(null); setStopTarget(null); setAuthTarget(null); load() }

  return (
    <>
      {addOpen && isUserAdmin && (
        <AddMaintenanceDialog rooms={rooms} types={types} onClose={() => setAddOpen(false)} onSaved={onDialogSaved} getAccessToken={getAccessToken} />
      )}
      {startTarget && isCleaningOperator && (
        <StartMaintenanceDialog log={startTarget} onClose={() => setStartTarget(null)} onSaved={onDialogSaved} getAccessToken={getAccessToken} />
      )}
      {stopTarget && isCleaningOperator && (
        <StopMaintenanceDialog log={stopTarget} onClose={() => setStopTarget(null)} onSaved={onDialogSaved} getAccessToken={getAccessToken} />
      )}
      {authTarget && isSysAdmin && (
        <AuthDialog log={authTarget.log} mode={authTarget.mode} onClose={() => setAuthTarget(null)} onSaved={onDialogSaved} getAccessToken={getAccessToken} />
      )}

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatsCard label="Total Records"     value={logs.length}  className="bg-sky-500/10 text-sky-600"        icon={Wrench} />
      ...
      <div className="rounded-xl border bg-card">
        ...
        <div className="flex flex-wrap items-center gap-2 px-4 py-3 border-b">
          <div className="relative flex-1 min-w-[180px] max-w-xs">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search rooms, type, reason…" className="h-8 w-full rounded-md border bg-background pl-8 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
          ...
          {(["all", "active", "scheduled", "stopped", "cancelled"] as const).map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={cn("h-8 rounded-full border px-3 text-xs font-medium transition-colors", statusFilter === s ? "border-sky-600 bg-sky-50 text-sky-700" : "border-border text-muted-foreground hover:border-sky-400")}>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-20 gap-2 text-muted-foreground"><Loader2 className="size-5 animate-spin" /><span className="text-sm">Loading…</span></div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-20 gap-2 text-red-600"><AlertCircle className="size-6" /><p className="text-sm font-medium">{error}</p><Button variant="outline" size="sm" onClick={load}>Retry</Button></div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-2 text-muted-foreground">
            <Wrench className="size-8 opacity-30" />
            <p className="text-sm">{logs.length === 0 ? (isUserAdmin ? "No maintenance records yet. Click 'Add Maintenance' to get started." : "No maintenance records to display.") : "No records match your filters."}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  {["Room ID","Room Name","Maintenance Type","Status","Approval","Start Time","End Time","Duration",...(!isUserAdmin ? ["Requested By","Action"] : [])].map((h) => (
              ...
              <tbody className="divide-y">
                {filtered.map((log) => (
                  <tr key={log.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3"><StatusBadge status={log.status as MaintenanceStatus} /></td>
                    <td className="px-4 py-3"><AuthBadge auth={log.authorizationStatus} /></td>
                    <td className="px-4 py-3 whitespace-nowrap text-xs">
                      {(log.status === "active" || log.status === "stopped") ? formatDt(log.maintenanceStartDatetime) : <span className="text-muted-foreground italic">Not started</span>}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-xs">
                      {log.maintenanceEndDatetime ? formatDt(log.maintenanceEndDatetime) : <span className="text-muted-foreground italic">—</span>}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-xs text-muted-foreground">{formatDuration(log.durationMinutes)}</td>
                    {!isUserAdmin && <td className="px-4 py-3 whitespace-nowrap text-xs text-muted-foreground">{log.markedByName}</td>}
                    {!isUserAdmin && <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {isSysAdmin && log.authorizationStatus === "pending" && log.status !== "cancelled" && (
                          <>
                            <Button variant="outline" size="sm" className="h-7 px-2 text-xs text-emerald-600 border-emerald-200 hover:bg-emerald-50" onClick={() => setAuthTarget({ log, mode: "approve" })}>
                              <ShieldCheck className="size-3 mr-1" />Approve
                            </Button>
                            <Button variant="outline" size="sm" className="h-7 px-2 text-xs text-red-600 border-red-200 hover:bg-red-50" onClick={() => setAuthTarget({ log, mode: "reject" })}>
                              <ShieldX className="size-3 mr-1" />Reject
                            </Button>
                          </>
                        )}
                        {isCleaningOperator && log.authorizationStatus === "approved" && log.status === "scheduled" && (
                          <Button variant="outline" size="sm" className="h-7 px-2 text-xs text-emerald-600 border-emerald-200 hover:bg-emerald-50" onClick={() => setStartTarget(log)}>
                            <Play className="size-3 mr-1" />Start
                          </Button>
                        )}
                        {isCleaningOperator && log.status === "active" && (
                          <Button variant="outline" size="sm" className="h-7 px-2 text-xs text-red-600 border-red-200 hover:bg-red-50" onClick={() => setStopTarget(log)}>
                            <StopCircle className="size-3 mr-1" />Stop
                          </Button>
                        )}
                        {!isSysAdmin && !isCleaningOperator && <span className="text-xs text-muted-foreground">—</span>}
                      </div>
                    </td>}
                  </tr>
                ))}
