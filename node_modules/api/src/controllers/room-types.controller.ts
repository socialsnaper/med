...
import {
  ...
  ImportRoomTypePayloadSchema,
} from '../validation/room-types.schemas';
...
export async function exportRoomTypesController(
...
export async function importRoomTypesController(
  req: Request, res: Response, next: NextFunction,
): Promise<void> {
  try {
    const { rows } = ImportRoomTypePayloadSchema.parse(req.body);
