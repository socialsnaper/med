...
import type { CreateRoomTypeInput, UpdateRoomTypeInput, ImportRoomTypeRow } from '../validation/room-types.schemas';
...
export async function streamRoomTypesCsv(
  schemaName: string,
  res: Response,
): Promise<void> {
  const db   = getPrismaClient(schemaName);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      await createRoomType(
        { roomTypeName: row.roomTypeName, roomTypeDetails: row.roomTypeDetails, displayOrder: row.displayOrder },
        schemaName,
        userId,
      );
      result.created++;
    } catch (err) {
      if (err instanceof RoomTypeError && err.code === 'DUPLICATE_NAME') result.skipped++;
